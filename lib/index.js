      ...
      upload: (file) => {
        return new Promise((resolve, reject) => {
          function process(file){
            ...
            file.tempBuffer = file.buffer.slice(); //use for thumb, as azure sdk deletes the buffer prop
          ...
          if([jimp.MIME_PNG, jimp.MIME_JPEG, 'image/jpg', jimp.MIME_BMP].indexOf(file.mime) > -1){
            return process(file)
            .then(function(){
              return jimp.read(file.tempBuffer)
              .then(function(image){
                file.buffer = file.tempBuffer;
